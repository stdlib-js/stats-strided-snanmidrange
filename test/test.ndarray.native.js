...
var snanmidrange = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( snanmidrange instanceof Error )
};
...
tape( 'the function has an arity of 4', opts, function test( t ) {
	t.strictEqual( snanmidrange.length, 4, 'has expected arity' );
	t.end();
});
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = snanmidrange( 0, x, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );

	v = snanmidrange( -1, x, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
